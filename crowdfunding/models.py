...
class Subsession(BaseSubsession):
    ...
    def creating_session(self):
        ...
        # Erstellt (Sitzungs-)Variable csv_file, welche folgenden Wert zugewiesen bekommt: Aktuelles Datum und Uhrzeit als String (z.B. "20.04.2020_14.09")
        ...
        for g in self.get_groups():
            ...
            with open(self.session.vars['csv_file'] + "_Group" + str(g.group_id) + ".csv", "w") as file:
                ...
                # Schreibt Folgendes in die CSV-Datei: WERTE der Variablen in settings.py, welche der Admin beim Erstellen der Sitzung festlegt + Anzahl Runden + Anzahl Spieler pro Gruppe + zwei neue Absätze (Inhalte getrennt durch Semikolons)
            ...
            with open(self.session.vars['csv_file'] + ".csv", "w") as file:
                ...
                # Schreibt Folgendes in die CSV-Datei: NAMEN der Variablen in settings.py, welche der Admin beim Erstellen der Sitzung festlegt + "num_rounds" + "players_per_group" (Inhalte getrennt durch Semikolons)
                file.write(
                    "All_Or_Nothing;Refund_Bonus;Early_Bird_Bonus;Enable_Backout;Backout_Fees;Show_Backers;Public_Goods;Min_Factor;Round_Duration;Bonus_Duration;Num_Projects;Endowment;Project_A_Goal;Project_B_Goal;Project_C_Goal;Project_D_Goal;Project_A_Seed;Project_B_Seed;Project_C_Seed;Project_D_Seed;real_world_currency_per_point;participation_fee;num_rounds;players_per_group" + "\n")

                # Schreibt Folgendes in die CSV-Datei: WERTE der Variablen in settings.py, welche der Admin beim Erstellen der Sitzung festlegt + Anzahl der Runden + Anzahl der Spieler pro Gruppe + zwei neue Absätze (Inhalte getrennt durch Semikolons)
                # Hinweis: Für Erklärungen der Variablen siehe SESSION_CONFIGS in settings.py
                file.write(str(self.session.config['All_Or_Nothing']) + ";" +
                           str(self.session.config['Refund_Bonus']) + ";" +
                           str(self.session.config['Early_Bird_Bonus']) + ";" +
            ...
            type_7 = [50, 100, 150, 200]

            # Liste, welche alle Präferenztypen enthält
            ...
            for p in g.get_players():
                ...
                # Erstellt Liste type_y mit folgendem Wert: Präferenztyp, welcher sich in der Liste types an der Stelle mit dem Index index befindet
                type_y = types[index]

                # Die mögliche Auszahlung des Spielers für das Projekt A wird auf folgenden Wert gesetzt: Wert, welcher sich in der Liste type_y an der Stelle mit Index 0 befindet
                p.a_payoff = type_y[0]

                # Die mögliche Auszahlung des Spielers für das Projekt B wird auf folgenden Wert gesetzt: Wert, welcher sich in der Liste type_y an der Stelle mit Index 1 befindet
                p.b_payoff = type_y[1]

                # Die mögliche Auszahlung des Spielers für das Projekt C wird auf folgenden Wert gesetzt: Wert, welcher sich in der Liste type_y an der Stelle mit Index 2 befindet
                p.c_payoff = type_y[2]

                # Die mögliche Auszahlung des Spielers für das Projekt D wird auf folgenden Wert gesetzt: Wert, welcher sich in der Liste type_y an der Stelle mit Index 3 befindet
                p.d_payoff = type_y[3]
...
class Group(BaseGroup):
    ...
    # x_backers: Anzahl der Spieler, welche das Projekt X unterstützen
    # Anfangswert: 0
    ...
    # Empfängt die Daten des JavaScript Codes in Crowdfunding.html, welche per liveSend() Methode gesendet werden
    ...
    # id_in_group: ID des Spielers, welcher einen Button ausgelöst hat, innerhalb seiner Gruppe
    # payload: Enthält Information, welcher Button ausgelöst wurde ("type") und falls es ein Donate Button war, welcher Betrag gespendet werden soll ("value")
    def live_method(self, id_in_group, payload):
        ...
        if payload["type"] == "donate_A":
            ...
            # Wählt den Spieler, welcher den Button ausgelöst hat, anhand seiner ID in der Gruppe
            ...
            if 0 < int(payload["value"]) <= p.money:
                ...
                # Falls der Betrag, welchen der Spieler bisher an das Projekt A spendet, gleich 0 ist:
                if p.a_contribution == 0:

                    # Erhöht die Anzahl der Spieler, welche das Projekt A unterstützen, um 1
                    self.a_backers += 1

                # Erhöht den Betrag, welchen der Spieler bisher an das Projekt A spendet, um den gewählten Wert
                ...
                # Öffnet die CSV-Datei, welche zu der Gruppe gehört, in der sich der Spieler befindet
                ...
                with open(self.session.vars['csv_file'] + "_Group" + str(p.group.group_id) + ".csv", "a") as file:
                    ...
                    file.write(
                               ...
                               # Sekunden, welche seit dem Start der aktuellen Crowdfunding Runde vergangen sind
        ...
        if payload["type"] == "backout_A":
            ...
            # Falls der Betrag, welchen der Spieler bisher an das Projekt A spendet, größer als 0 ist:
            if p.a_contribution > 0:

                # Erhöht das Kapital des Spielers um den Betrag, welchen er aktuell an das Projekt A spendet
                ...
                # Verringert die Summe der Spenden aller Spieler für das Projekt A um den Betrag, welchen der Spieler aktuell an das Projekt A spendet
                self.a_total -= p.a_contribution

                # Verringert die Anzahl der Spieler, welche das Projekt A unterstützen, um 1
                self.a_backers -= 1

                # Variable a_contribution_csv wird erstellt und bekommt folgenden Wert zugewiesen: Betrag, welchen der Spieler aktuell an das Projekt A spendet
                # Hinweis: Dient zum Erstellen der CSV-Dateien der einzelnen Gruppen. Dokumentiert, welche Beträge von Spielern zurückgezogen werden
                a_contribution_csv = p.a_contribution

                # Setzt den Betrag, welchen der Spieler aktuell an das Projekt A spendet, auf 0
                p.a_contribution = 0
                ...
                with open(self.session.vars['csv_file'] + "_Group" + str(p.group.group_id) + ".csv", "a") as file:
                    ...
                    file.write(
                        ...
                        # Betrag, welcher zurückgezogen wird (VOR Abzug eventueller Kosten für das Aussteigen)
...
class Player(BasePlayer):
    ...
    # Anfangswert: Wert, welcher beim Erstellen der Sitzung vom Admin gewählt wird (Zuweisung durch creating_session() Methode in Subsession Klasse)
    ...
    # x_early_bird: Early Bird Bonus, welchen der Spieler für das Projekt x erhält
    # Anfangswert: 0,0
    ...
    # x_refund_bonus: Refund Bonus, welchen der Spieler erhält, wenn das Projekt x das Spendenziel nicht erreicht
    # Anfangswert: 0,0
    ...
    # total_payoff: Gibt die Auszahlung an, welche der Spieler am Ende des Experimentes insgesamt erreicht hat (Übungsrunde fließt nicht mit ein)
    ...
    def set_payoff(self):
        ...
        # Falls die Projekte öffentliche Güte sind:
        ...
        # Andernfalls (Projekte sind keine öffentlichen Güter)
        else:
            ...
            if self.session.config['All_Or_Nothing']:
                ...
                # Falls das Spendenziel von Projekt A (am Ende der Runde) erreicht ist...
                # ...und der Betrag, welchen der Spieler an Projekt A gespendet hat, mindestens so groß ist wie das Produkt aus dem Wert der Variable factor und dem Spendenziel von Projekt A:
