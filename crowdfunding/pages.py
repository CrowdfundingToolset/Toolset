...
class Instructions(Page):
    ...
    def vars_for_template(self):
        ...
        return{
            ...
            # Gibt an, ob die Anzahl der Spieler, die ein Projekt unterstützen, angezeigt werden soll (True oder False)
            ...
            # Gibt an, ob bei Projekterfolgen öffentliche Güter (True) oder private Güter (False) entstehen sollen
            ...
            # Zeitraum, in denm Boni gewährt werden, in Sekunden
...
class Crowdfunding(Page):
    ...
    def vars_for_template(self):
        ...
        return{
            ...
            # Nur relevant, wenn bei Projekterfolgen private Güter entstehen sollen ("Public_Goods" in settings.py ist deaktiviert)
            # x_minimum: Minimaler Betrag, den ein Spieler an das Projekt X spenden muss, um seine zugehörige Auszahlung (im Falle des Projekterfolges) zu erhalten
...
class ResultsWaitPage(WaitPage):
    ...
    def after_all_players_arrive(self):
        ...
        for p in self.group.get_players():
            ...
            # Öffnet die CSV Datei, in der die Rundenergebnisse gespeichert werden
            ...
            with open(self.session.vars['csv_file'] + ".csv", "a") as file:
                ...
                if p.group.a_funded:
                    ...
                    # Andernfalls (Bei Projekterfolgen sollen private Güter entstehen)
                        ...
                        # Falls der Spieler mindestens den Betrag an das (erfolgreiche) Projekt A gespendet hat, der nötig ist, um die zugehörige Auszahlung zu erhalten:
                ...
                # Andernfalls (Projekt A hat das Spendenziel nicht erreicht)
                    ...
                    if self.session.config['All_Or_Nothing']:
                        ...
                        # Erstellt Variable a_refund mit dem Wert: Betrag, den der Spieler an Projekt A gespendet hat + Rückzahlungs-Bonus des Spielers für Projekt A
                ...
                file.write(
                    ...
                    # ID der Gruppe, in der sich der Spieler befindet
                    ...
                    # Höhe der TATSÄCHLICHEN Auszahlung, die der Spieler für Projekt A erhält
                    str(a_payoff) + ";" +

                    # Höhe der TATSÄCHLICHEN Auszahlung, die der Spieler für Projekt B erhält
                    str(b_payoff) + ";" +

                    # Höhe der TATSÄCHLICHEN Auszahlung, die der Spieler für Projekt C erhält
                    str(c_payoff) + ";" +

                    # Höhe der TATSÄCHLICHEN Auszahlung, die der Spieler für Projekt D erhält
                    str(d_payoff) + ";" +

                    # Höhe der Rückzahlung, die der Spieler für Projekt A erhält
                    str(a_refund) + ";" +

                    # Höhe der Rückzahlung, die der Spieler für Projekt B erhält
                    str(b_refund) + ";" +

                    # Höhe der Rückzahlung, die der Spieler für Projekt C erhält
                    str(c_refund) + ";" +

                    # Höhe der Rückzahlung, die der Spieler für Projekt D erhält
                    str(d_refund) + ";" +

                    # Höhe des Rückzahlungs-Bonus, den der Spieler für Projekt A erhält
                    str(p.a_refund_bonus) + ";" +

                    # Höhe des Rückzahlungs-Bonus, den der Spieler für Projekt B erhält
                    str(p.b_refund_bonus) + ";" +

                    # Höhe des Rückzahlungs-Bonus, den der Spieler für Projekt C erhält
                    str(p.c_refund_bonus) + ";" +

                    # Höhe des Rückzahlungs-Bonus, den der Spieler für Projekt D erhält
                    str(p.d_refund_bonus) + ";" +

                    # Höhe des Early-Bird-Bonus, den der Spieler für Projekt A erhält
                    str(p.a_early_bird) + ";" +

                    # Höhe des Early-Bird-Bonus, den der Spieler für Projekt B erhält
                    str(p.b_early_bird) + ";" +

                    # Höhe des Early-Bird-Bonus, den der Spieler für Projekt C erhält
                    str(p.c_early_bird) + ";" +

                    # Höhe des Early-Bird-Bonus, den der Spieler für Projekt D erhält
                    str(p.d_early_bird) + ";" +

                    # Höhe der MÖGLICHEN Auszahlung, die der Spieler für Projekt A hätte erhalten können
                    str(p.a_payoff) + ";" +

                    # Höhe der MÖGLICHEN Auszahlung, die der Spieler für Projekt B hätte erhalten können
                    str(p.b_payoff) + ";" +

                    # Höhe der MÖGLICHEN Auszahlung, die der Spieler für Projekt C hätte erhalten können
                    str(p.c_payoff) + ";" +

                    # Höhe der MÖGLICHEN Auszahlung, die der Spieler für Projekt D hätte erhalten können
                    str(p.d_payoff) + ";" +
                    ...
                    # Betrag, den der Spieler an Projekt B gespendet hat
                    str(p.b_contribution) + ";" +

                    # Betrag, den der Spieler an Projekt C gespendet hat
                    str(p.c_contribution) + ";" +

                    # Betrag, den der Spieler an Projekt D gespendet hat
                    ...
                    # Anzahl der Spieler, die das Projekt A unterstützen
                    str(p.group.a_backers) + ";" +

                    # Anzahl der Spieler, die das Projekt B unterstützen
                    str(p.group.b_backers) + ";" +

                    # Anzahl der Spieler, die das Projekt C unterstützen
                    str(p.group.c_backers) + ";" +

                    # Anzahl der Spieler, die das Projekt D unterstützen
...
# Seite, die dem Spieler die Ergebnisse der aktuellen Runde anzeigt
class Results(Page):
    ...
    def vars_for_template(self):
        ...
        # Andernfalls (Projekt A hat das Spendenziel nicht erreicht)
            ...
            if self.session.config['All_Or_Nothing']:
                ...
                # Erstellt Variable a_payoff mit dem Wert: Betrag, den der Spieler an Projekt A gespendet hat + Rückzahlungs-Bonus für Projekt A
        ...
        return{
            ...
            # Nur relevant, wenn bei Projekterfolgen private Güter entstehen sollen ("Public_Goods" in settings.py ist deaktiviert)
            # x_min_boolean: Gibt an, ob der Spieler mindestens den Betrag an das Projekt X gespendet hat, der nötig ist, um die zugehörige Auszahlung (im Falle des Projekterfolges) zu erhalten (True oder False)
            # Hinweis: x ∈ {a, b, c, d}
        ...
        for p in self.group.get_players():
            ...
            for entry in p.in_all_rounds():
                ...
                if entry.round_number != 1:
                    ...
                    # Erhöht die insgesamt erreichte Auszahlung des Spielers um die Auszahlung aus der Runde, die aktuell repräsentiert wird
                ...
                for g in self.subsession.get_groups():
                    ...
                    for p in g.get_players():
                        ...
                        file.write(
                            ...
                            # Auszahlung, die der Spieler im gesamten Experiment erreicht hat (ohne Übungsrunde)
                            str(p.total_payoff) + ";" +
...
# Seite, die dem Spieler die Ergebnisse des kompletten Experiments anzeigt
class End(Page):
    ...
    def vars_for_template(self):
        ...
        for entry in self.player.in_all_rounds():
            ...
            # Fügt die Auszahlung aus der Runde, die aktuell repräsentiert wird, der Liste payoff_list hinzu
